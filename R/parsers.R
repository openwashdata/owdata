...
#' Normalize a version string for the catalog
#'
#' The data model stores the version with any leading "v" stripped (#5).
#' One package in the org writes "v1.0.0" in DESCRIPTION. The parser keeps
#' the field verbatim; this is what the harvest writes to the table.
#'
#' @param version Character vector of raw version strings.
#' @return Character vector, leading v removed, empty strings as NA.
#' @keywords internal
#' @noRd
...
owd_parse_dictionary <- function(text) {
  ...
  # Some dictionaries in the org are latin1. Every later call, trimws()
  # included, raises "invalid UTF-8" on those bytes, and a parser may never
  # abort a harvest run, so the conversion happens before anything reads the
  # string.
  if (!validUTF8(text)) {
    converted <- iconv(text, from = "latin1", to = "UTF-8")
    if (is.na(converted) || !validUTF8(converted)) {
      res$notes <- "file is neither valid UTF-8 nor latin1"
      return(res)
    }
    text <- converted
    res$notes <- c(res$notes, "converted latin1 to UTF-8")
  }
  if (!nzchar(trimws(text))) {
    res$notes <- c(res$notes, "empty or missing file")
    return(res)
  }
  ...
  # washr before 1.1.0 wrote a multi-class variable_type as a deparsed
  # vector, for example c("POSIXct", "POSIXt"). Keep the first class only,
  # so the column holds one class name whatever wrote it.
  multi <- !is.na(df$variable_type) & grepl("^c\\(", df$variable_type)
  if (any(multi)) {
    df$variable_type[multi] <- sub(
      '^c\\(\\s*[\\\\"\']*([^,\\\\"\')]+).*$', "\\1", df$variable_type[multi]
    )
    df$variable_type[multi] <- trimws(df$variable_type[multi])
    res$notes <- c(res$notes, sprintf(
      "normalized %d multi-class variable_type value(s) to the first class",
      sum(multi)
    ))
  }
