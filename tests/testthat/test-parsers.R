...
# Read the fixture as raw bytes, the way a harvest backend hands the text
# over, so an encoding declaration cannot paper over the file's contents.
read_fixture_raw <- function(name) {
  path <- test_path("fixtures", name)
  rawToChar(readBin(path, "raw", file.size(path)))
}

test_that("non-ASCII characters survive as themselves, never as escapes", {
  res <- owd_parse_dictionary(read_fixture_raw("dictionary-non-ascii.csv"))
  expect_equal(res$status, "ok")
  expect_true(all(validUTF8(res$data$description)))
  expect_false(any(grepl("<U\\+", res$data$description)))
  expect_true(any(grepl("Zürich", res$data$description, fixed = TRUE)))
  expect_true(any(grepl("Qualité", res$data$description, fixed = TRUE)))
})

test_that("a latin1 dictionary is converted, not aborted", {
  res <- owd_parse_dictionary(read_fixture_raw("dictionary-latin1.csv"))
  expect_equal(res$status, "ok")
  expect_true(any(grepl("converted latin1", res$notes)))
  expect_true(all(validUTF8(res$data$description)))
  expect_false(any(grepl("<U\\+", res$data$description)))
  expect_true(any(grepl("Zürich", res$data$description, fixed = TRUE)))
})

test_that("a multi-class variable_type is normalized to its first class", {
  res <- owd_parse_dictionary(read_fixture_raw("dictionary-non-ascii.csv"))
  expect_equal(res$status, "ok")
  expect_false(any(grepl("^c\\(", res$data$variable_type)))
  expect_true("POSIXct" %in% res$data$variable_type)
  expect_true(any(grepl("multi-class", res$notes)))
})
...
test_that("a leading v survives parsing and is stripped for the catalog", {
