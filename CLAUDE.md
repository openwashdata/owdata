...
CSV for openwashdata.org. It is an engine only. The harvest is a script,
and there is no owdata skill.
...
- `data-raw/harvest.R`, `harvest_helpers.R`: the harvest and its
  backends
- `data-raw/packages.json`, `harvest_report.md`: generated
- `tests/testthat/`: tests and `fixtures/` with real pathological files
- `.github/workflows/`: R-CMD-check, pkgdown, test-coverage, harvest
- `_pkgdown.yml`, `_brand.yml`, `logos/`: written by washr, refreshed
  with `washr::use_brand()`

Never hand-edit generated files: `R/sysdata.rda`, `inst/extdata/`,
`man/`, `NAMESPACE`, `data-raw/packages.json`,
`data-raw/harvest_report.md`. Until \#3 to \#6 land, `R/` holds only the
package doc.
