...
owd_gh_token <- function() {
  ...
  # In Actions the token arrives as an environment variable. On a
  # maintainer's machine it usually lives in the gh CLI keyring instead,
  # where an unauthenticated run would otherwise hit the 60 requests per
  # hour limit for anonymous callers.
  tok <- suppressWarnings(tryCatch(
    system2("gh", c("auth", "token"), stdout = TRUE, stderr = FALSE),
    error = function(e) character()
  ))
  if (length(tok) > 0 && nzchar(tok[1])) {
    return(trimws(tok[1]))
  }
  ""
}
