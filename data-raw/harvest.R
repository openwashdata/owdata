...
#   inst/extdata/owd_{packages,datasets,variables}.csv
...
harvest_repo <- function(src) {
  ...
  # A package is published when CITATION.cff carries a Zenodo DOI. There
  # are no version tiers (dev/decisions-2026-09.md, section 1).
  published <- !is.na(cff$doi) && grepl("^10\\.5281/zenodo\\.", cff$doi)
  ...
  # schema.org fields washr 1.1.0 writes into DESCRIPTION (#5).
  dcf <- tryCatch(read.dcf(textConnection(desc_text)), error = function(e) NULL)
  desc_field <- function(name) {
    if (!is.null(dcf) && name %in% colnames(dcf) && !is.na(dcf[1, name])) {
      trimws(gsub("\\s+", " ", unname(dcf[1, name])))
    } else {
      NA_character_
    }
  }
  keywords <- desc_field("X-schema.org-keywords")
  if (is.na(keywords)) keywords <- src$meta$topics

  # Tooling provenance (#18). washr_version stays NA until washr #81
  # ships the Config/washr fields.
  workflows <- src$list_files(".github/workflows")
  claude_md <- src$read_file("CLAUDE.md")
  pkgreview_standard <- NA_character_
  if (!is.na(claude_md)) {
    # Split first: a character class cannot carry the newline escapes,
    # so matching against the whole file would run past the line end.
    stamp <- grep("Standard version:", strsplit(claude_md, "\r?\n")[[1]], value = TRUE)
    if (length(stamp) > 0) {
      pkgreview_standard <- trimws(sub("^.*Standard version:\\s*", "", stamp[1]))
    }
  }
  brand_ref <- if (!is.na(desc_field("Config/washr/brand"))) {
    desc_field("Config/washr/brand")
  } else if (!is.na(src$read_file("_brand.yml"))) {
    "main"
  } else {
    NA_character_
  }
  ...
  package_row <- tibble::tibble(
    ...
    date = desc$date,
    keywords = keywords,
    spatial_coverage = desc_field("X-schema.org-spatialCoverage"),
    temporal_coverage = desc_field("X-schema.org-temporalCoverage"),
    doi = cff$doi,
    ...
    washr_version = desc_field("Config/washr/version"),
    brand_ref = brand_ref,
    pkgreview_standard = pkgreview_standard,
    has_r_cmd_check = "R-CMD-check.yaml" %in% workflows,
    has_pkgdown_workflow = "pkgdown.yaml" %in% workflows,
    has_jsonld = !is.na(src$read_file("pkgdown/templates/in-header.html"))
  )
...
if (any(packages$published) &&
    (is.null(datasets) || is.null(variables) ||
     nrow(datasets) == 0 || nrow(variables) == 0)) {
  stop("Published packages exist but the detail tables are empty; refusing to write the catalog.")
}
if (is.null(datasets)) {
  datasets <- tibble::tibble(
...
if (is.null(variables)) {
  variables <- tibble::tibble(
    pkg_name = character(), dataset_name = character(), variable_name = character(),
    variable_type = character(), description = character()
  )
}

# --- value-level gates (#4) -------------------------------------------------
# These catch defects the schema tests cannot see, because NA and a
# mojibake string are both structurally valid.

escaped <- unlist(lapply(list(packages, datasets, variables), function(df) {
  chr <- vapply(df, is.character, logical(1))
  unlist(lapply(df[chr], function(x) x[!is.na(x) & grepl("<U\\+[0-9A-Fa-f]{4}", x)]))
}))
if (length(escaped) > 0) {
  stop(sprintf(
    "Found %d value(s) carrying <U+xxxx> escapes; the run needs a UTF-8 locale. First: %s",
...
if (length(dup_pkg) > 0) {
  stop("Duplicated package name(s): ", paste(unique(dup_pkg), collapse = ", "))
}
...
if (file.exists(prev_path)) {
  ...
  old_pub <- toupper(prev$published[match(both, prev$pkg_name)])
  new_pub <- toupper(as.character(packages$published[match(both, packages$pkg_name)]))
  moved <- both[!is.na(old_pub) & old_pub != new_pub]
  for (p in moved) {
    deltas <- c(deltas, sprintf(
      "published change: %s %s -> %s", p,
      old_pub[match(p, both)], new_pub[match(p, both)]
    ))
  }
...
# Read the committed CSVs before overwriting them, so the version bump
# below can tell a real change from a no-op run.
previous_csv <- lapply(names(tables), function(name) {
  path <- file.path("inst/extdata", paste0(name, ".csv"))
  if (file.exists(path)) readLines(path, warn = FALSE) else NULL
})
names(previous_csv) <- names(tables)
...
# --- version bump (#4) ------------------------------------------------------
# Any change to the shipped tables bumps the fourth component and sets
# Date, so r-universe rebuilds and update.packages() sees a new version.

changed <- vapply(names(tables), function(name) {
  path <- file.path("inst/extdata", paste0(name, ".csv"))
  !identical(previous_csv[[name]], if (file.exists(path)) readLines(path, warn = FALSE) else NULL)
}, logical(1))
if (any(changed)) {
  desc::desc_bump_version("dev", file = "DESCRIPTION")
  desc::desc_set("Date", as.character(meta$harvest_date), file = "DESCRIPTION")
  message("Version bumped to ", desc::desc_get_version(file = "DESCRIPTION"))
} else {
  message("Tables unchanged; version left as it is.")
}
...
report <- c(
  ...
  sprintf(
    "Packages: %d (%d published). Datasets: %d. Variables: %d.",
    nrow(packages), sum(packages$published), nrow(datasets), nrow(variables)
  ), "",
