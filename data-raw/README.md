...
- `harvest.R`: orchestrator. Enumerate repos, detect data packages,
  parse metadata, set the published flag, assemble the catalog, run
  gates, write all outputs.
...
columns, published-flag changes, dropped packages) so silent
...
1. The plan named both data objects `owd_packages` and accessor
   ...
   API.
2. The plan placed the parsers in `data-raw/`, but their unit tests must
   run during `R CMD check` of the built tarball, which does not contain
   `data-raw/`. The pure functions therefore live in `R/parsers.R` and
   `R/catalog.R` (internal, tested), while everything that touches the
   network or filesystem stays here.
